...
const STATS = [
  { raw: '12+', label: 'Projects Built', color: '#00d4ff', pct: 80 },
  { raw: '4+', label: 'AI Domains', color: '#a855f7', pct: 70 },
  { raw: '7.93', label: 'CGPA / 10.0', color: '#06ffa5', pct: 79 },
  { raw: '2+', label: 'Years in ML', color: '#ff6b6b', pct: 65 },
]

const EXPERTISE = [
  'Machine Learning', 'Deep Learning', 'LLMs & RAG',
  'NLP', 'Computer Vision', 'FastAPI', 'Docker', 'Flutter AI',
  'PyTorch', 'HuggingFace', 'FAISS', 'llama.cpp',
]

// Circumference for r=28: 2*pi*28 ≈ 175.9
const CIRC = 175.9

function StatCard({ raw, label, color, pct }) {
  ...
  return (
    <div
      ref={ref}
      className={`stat-card-3d ${visible ? 'reveal' : ''}`}
      style={{ '--stat-color': color, '--ring-offset': offset }}
    >
      {/* Circular progress ring */}
      <div className="stat-ring-wrap">
        <svg className="stat-ring-svg" viewBox="0 0 64 64">
